...
class User(object):
    ...
    def __init__(self, user_data: dict) -> None:
        self.group_membership_id = (
            user_data.get("id", None) if user_data.get("user", None) else None
        )
    ...
    def __init__(self, data: dict) -> None:
        ...
        try:
            self.creation_date = datetime.strptime(
                data.get("creation_date", None), datetime_format
            )
        except:
            self.creation_date = None
        try:
            self.version_date = datetime.strptime(
                data.get("version_date", None), datetime_format
            )
        except:
...
class LabFolder(object):
    ...
    def _get_logged_user(self, headers: dict) -> User:
        ...
        r = GET(f"{self._api_base_url}/me", params={"expand": "user"}, headers=headers)
    ...
    def login(self, username: str) -> None:
        ...
        if self.me:
            raise Exception(
                "You are already logged in. Log out before logging in again."
            )
        ...
        # Send request
        r = POST(f"{self._api_base_url}/auth/login", json=credentials)
        ...
        if r.status_code == 200:
            token = r.json()["token"]
            headers = {
                "User-Agent": f"LabFolderApi; {username}",
    ...
    def logout(self) -> None:
        ...
        # Send request
        r = POST(f"{self._api_base_url}/auth/logout", headers=self.me._headers)
    ...
    def get_group(self, group_id: int) -> Group:
        ...
        # Send request
        r = GET(
            f"{self._api_base_url}/groups/{group_id}/tree", headers=self.me._headers
        )
    ...
    def get_entries_projects(self, user: User = None, limit: int = 0) -> list:
        ...
        while True:
            # Send request
            r = GET(
                f"{self._api_base_url}/entries",
                params={"project_ids": project_ids, "limit": limit, "offset": offset},
    ...
    def _get_records(
        self, rec_obj: Union[Project, Folder], user: User = None, limit: int = 0
    ) -> list:
        """
        ...
        while True:
            # Send request
            r = GET(
                f"{self._api_base_url}/{record_type}",
                params={"owner_id": user.id, "limit": limit, "offset": offset},
                headers=self.me._headers,
            )
    ...
    def set_owner(self, record: Union[Folder, Project], new_owner: User) -> None:
        ...
        if rec_obj_name not in ["Folder", "Project"]:
            raise Exception(
                f'Setting the owner for "{rec_obj_name}" is not yet suported.'
            )
        ...
        # Send request
        r = PATCH(
            f"{self._api_base_url}/{record_type}/{record.id}",
            data=data,
    ...
    def remove_user_from_group(self, user: User) -> None:
        ...
        # Send request
        r = DELETE(
            f"{self._api_base_url}/group-memberships/{user.group_membership_id}",
    ...
    def export_as_pdf(self, record: Union[Entry, Project]) -> None:
        ...
        data = {
            ...
            "settings": {"preserve_entry_layout": True},
            "content": {f"{rec_obj_name.lower()}_ids": [record.id]},
        }

        # Send request
        r = POST(
            f"{self._api_base_url}/exports/pdf", json=data, headers=self.me._headers
        )
